...
function launchParticlesJS(tag_id, params){
  ...
  pJS.fn.particle = function(color, opacity, position,type){
  ...
  pJS.fn.particlesCreate = function(){
    for(var i = 0; i < pJS.particles.nb; i++) {
      var shapeArr  = ["circle","circle","circle","circle"];
      var colorArr = ["#000000","#ffffff","#ffffff","#ffffff"];
      var m = Math.floor(Math.random()*3+1);
      console.log(m);
      var cor = hexToRgb(colorArr[m]);
      pJS.particles.array.push(new pJS.fn.particle(cor, pJS.particles.opacity,null,shapeArr[m]));
    }
  };
